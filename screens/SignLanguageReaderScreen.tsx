...
export default function SignLanguageReaderScreen() {
  ...
  const startCapture = useCallback(async () => {
    ...
    captureIntervalRef.current = setInterval(async () => {
      if (!isCapturingRef.current || !cameraRef.current || isSendingRef.current) return;
...
const styles = StyleSheet.create({
  ...
  confValue: { fontSize: 13, fontWeight: '700', color: DARK, width: 36, textAlign: 'right' },
  ...
  top5Pct: { fontSize: 12, fontWeight: '700', color: CYAN, width: 36, textAlign: 'right' },
